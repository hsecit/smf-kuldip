...
function list_getModLogEntries($start, $items_per_page, $sort, $query_string = '', $query_params = array(), $log_type = 1)
{
	...
	while ($row = $smcFunc['db_fetch_assoc']($result))
	{
        ...
        // Custom page?
        if(in_array($row['action'], array('add_page', 'edit_page')))
        {
            // Load the page and add its name to the extra array
            global $sourcedir;
            require_once($sourcedir . '/Subs-CustomPages.php');
            $page = loadPage($row['extra']['page']);
            if($page && allowedToViewPage($page['perms']))
                $row['extra']['page'] = $page;
        }
	...
	foreach ($entries as $k => $entry)
	{
        ...
        // Custom Page actions
        if ($entry['action'] == 'add_page')
            if (is_array($entry['extra']['page']))
                $entries[$k]['action_text'] = sprintf($txt['cp_add'], $entry['extra']['page']['id'], $entry['extra']['page']['title']);
            else
                $entries[$k]['action_text'] = sprintf($txt['cp_add2'], $entry['extra']['page']);
        elseif ($entry['action'] == 'edit_page')
            if (is_array($entry['extra']['page']))
                $entries[$k]['action_text'] = sprintf($txt['cp_edit'], $entry['extra']['page']['id'], $entry['extra']['page']['title']);
            else
                $entries[$k]['action_text'] = sprintf($txt['cp_edit2'], $entry['extra']['page']);
        elseif ($entry['action'] == 'remove_page')
            $entries[$k]['action_text'] = sprintf($txt['cp_remove'], $entry['extra']['page']);
