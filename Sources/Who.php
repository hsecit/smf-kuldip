...
function determineActions($urls, $preferred_prefix = false)
{
	...
	foreach ($url_list as $k => $url)
	{
		...
		else
		{
            ...
            // A Custom Page?
            elseif ($actions['action'] == 'page')
            {
                // Get some functions we need.
                global $sourcedir;
                require_once($sourcedir . '/Subs-CustomPages.php');
                
                // Load the page to see if it exists and get the name.
                $page = loadPage($actions['sa']);
                
                // Check if the user is allowed to view the page.
                if(allowedToViewPage($page['perms']) && $page !== false)
                    $data[$k] = sprintf($txt['page1'], $actions['sa'], $page['title']);
                // Otherwise just say they are viewing a custom page.
                else
                    $data[$k] = $txt['page2'];
            }
